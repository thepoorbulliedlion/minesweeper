...
//use js to make all boxes and assign ids, add click logic, detect wins
{
  for (let i = 1; i <= grid; i++) {
    ...
    for (let j = 1; j <= grid; j++) {
      ...
      //click stuff
      {
        text.addEventListener("mousedown", (test) => {
  ...
  for (let i = 1; i <= grid; i++) {
    for (let j = 1; j <= grid; j++) {
      ...
      if (
        ...
        squares[i][j].firstChild.addEventListener("mousedown", (mine) => {
          console.log("kaboom");
          if (mine.button == 0) {
...
for (let i = 0; i < zeroposx.length; i++) {
  ...
  document.getElementById(pos).firstChild.addEventListener("click", () => {
    console.log("clicked");
